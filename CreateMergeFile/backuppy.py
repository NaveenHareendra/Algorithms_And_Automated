    ...
    def CheckFileName(self, FilePath):
        ...
        try:
            if((charCheck[0]=="0")& (len(charCheck)-1>0) & (invalidFlag!=-1)):
                newPath=self.RenameFileName(FilePath=FilePath)
                stringPrepped =str(newPath)+','
                self.pathListMerge.append(stringPrepped)
                # print("File Starts with 0 File renamed : ",newPath)
            else:
                if(invalidFlag==-1):
            ...
            if(invalidFlag==-1):
                ...
                print("File verification: Something went wrong ...");
